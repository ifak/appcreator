...
void GeneralSettings::fillLanguageBox() const
{
  ...
  const QStringList languageFiles = QDir(creatorTrPath).entryList(QStringList(QLatin1String("appcreator*.qm")));
...
QWidget *GeneralSettings::createPage(QWidget *parent)
{
  ...
  this->m_page->graphvizPath->setText(qvariant_cast<QString>(settings->value(QLatin1String("GraphvizPath"))));
  settings->endGroup();
  QObject::connect(this->m_page->graphvizPathBrowseButton, &QPushButton::clicked, this, &GeneralSettings::browseGraphvizPath);
...
void GeneralSettings::apply()
{
  ...
  QSettings* settings = Core::ICore::settings();
  settings->beginGroup(QLatin1String("Graphviz"));
  settings->setValue(QLatin1String("GraphvizPath"),this->m_page->graphvizPath->text());
  settings->endGroup();
}
...
void GeneralSettings::browseGraphvizPath(){
    QString dir = QFileDialog::getExistingDirectory(new QWidget(),tr("select graphviz bin dir"),QLatin1String("/home"),QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if(dir != QLatin1String("")){
        this->m_page->graphvizPath->setText(dir);
    }
}
...
void GeneralSettings::setLanguage(const QString &locale)
{
  ...
  if (settings->value(QLatin1String("General/OverrideLanguage")).toString() != locale)
  {
    QMessageBox::information(Core::ICore::mainWindow(), tr("Restart required"),
                             tr("The language change will take effect after a restart of %1.")
