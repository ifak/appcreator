...
MainWindow::MainWindow(ApplicationUpdate* updater) :
  Utils::AppMainWindow(),
  m_update(updater),
  ...
  m_settingsDatabase(new SettingsDatabase(QFileInfo(m_settings->fileName()).path(),
                                          QLatin1String(Constants::IDE_APP_NAME),
                                          this)),
  m_printer(0),
  ...
  m_toggleSideBarButton(new QToolButton)
{
  ...
  this->setWindowTitle(QLatin1String(Constants::IDE_APP_NAME));
  if (!Utils::HostOsInfo::isMacHost())
    QApplication::setWindowIcon(QIcon(QLatin1String(Constants::IDE_APP_WINDOW_LOGO)));
  ...
  QCoreApplication::setOrganizationName(QLatin1String(Constants::IDE_AUTHOR));
...
void MainWindow::registerDefaultActions()
{
  ...
  if (Utils::HostOsInfo::isMacHost())
    tmpaction = new QAction(icon, tr("About &%1").arg(QLatin1String(Core::Constants::IDE_APP_NAME)), this); // it's convention not to add dots to the about menu
  else
    tmpaction = new QAction(icon, tr("About &%1...").arg(QLatin1String(Core::Constants::IDE_APP_NAME)), this);
  ...
  tmpaction = new QAction(tr("Check for Updates"), this);
  cmd = ActionManager::registerAction(tmpaction, Constants::CHECKFORUPDATES, globalContext);
  if (Utils::HostOsInfo::isMacHost())
    cmd->action()->setMenuRole(QAction::ApplicationSpecificRole);
  mhelp->addAction(cmd, Constants::G_HELP_ABOUT);
  tmpaction->setEnabled(true);
  connect(tmpaction, SIGNAL(triggered()), this,  SLOT(checkForUpdates()));
}
...
{
  //  qDebug()<<"MainWindow::addContextObject() executed!"<<context;
  if (!context)
    return;
  ...
  //  qDebug()<<"context object inserted with widget: "<<widget<<"and context: "<<context;
...
{
  //  qDebug()<<"MainWindow::updateFocusWidget() executed!";
  Q_UNUSED(old)
...
{
  //  qDebug()<<"MainWindow::updateContextObject executed! contextList: "<<context;
...
{
  //start updater
  if(this->manualUpdateRunning == true)
  {
    QMessageBox messageBox;
    messageBox.raise();
    messageBox.question(QApplication::activeWindow(),
                        QStringLiteral("Updates"),
                        QStringLiteral("Manual Update is already running!"),
                        QMessageBox::Ok);
    return;
  }
  this->manualUpdateRunning = true;
  QObject::connect(m_update, SIGNAL(finishUpdate()),
                   this, SLOT(allowManualUpdates()));
  QtConcurrent::run(m_update, &ApplicationUpdate::checkRepository, true);
}

void MainWindow::allowManualUpdates()
{
  this->manualUpdateRunning = false;
}
