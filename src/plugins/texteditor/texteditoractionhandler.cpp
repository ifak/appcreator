...
void TextEditorActionHandler::createActions()
{
  ...
  command = Core::ActionManager::registerAction(m_unCommentSelectionAction,
                                                Constants::UN_COMMENT_SELECTION,
                                                m_contextId,
                                                true);
  command->setDefaultKeySequence(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_Slash));
...
{
  if (m_redoAction)
    m_redoAction->setEnabled(m_currentEditor
                             && m_currentEditor->document()->isRedoAvailable());
}

void TextEditorActionHandler::updateUndoAction()
{
  if (m_undoAction)
    m_undoAction->setEnabled(m_currentEditor
                             && m_currentEditor->document()->isUndoAvailable());
}
