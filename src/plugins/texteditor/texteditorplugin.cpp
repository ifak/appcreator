...
bool TextEditorPlugin::initialize(const QStringList &arguments, QString *errorMessage)
{
  ...
  Core::Command *command = Core::ActionManager::registerShortcut(completionShortcut,
                                                                 Constants::COMPLETE_THIS,
                                                                 context);
  command->setDefaultKeySequence(QKeySequence(Core::UseMacShortcuts ? tr("Meta+Space") : tr("Ctrl+Space")));
  connect(completionShortcut, &QShortcut::activated, this, &TextEditorPlugin::invokeCompletion);
  ...
  Core::Command *quickFixCommand = Core::ActionManager::registerShortcut(quickFixShortcut,
                                                                         Constants::QUICKFIX_THIS,
                                                                         context);
