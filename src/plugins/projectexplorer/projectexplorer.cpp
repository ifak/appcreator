...
//  msessionContextMenu->addAction(cmd, Constants::G_SESSION_BUILD);
  ...
  cmd = Core::ActionManager::registerAction(d->m_buildAction,
                                            Constants::BUILD,
                                            globalcontext);
  cmd->setAttribute(Core::Command::CA_UpdateText);
...
//  mprojectContextMenu->addAction(cmd, Constants::G_PROJECT_RUN);
  ...
  cmd = Core::ActionManager::registerAction(d->m_addNewFileAction,
                                            ProjectExplorer::Constants::ADDNEWFILE,
                                            projecTreeContext);
  ...
  cmd = Core::ActionManager::registerAction(d->m_addExistingFilesAction,
                                            ProjectExplorer::Constants::ADDEXISTINGFILES,
                                            projecTreeContext);
  //    mprojectContextMenu->addAction(cmd, Constants::G_PROJECT_FILES);
  //    msubProjectContextMenu->addAction(cmd, Constants::G_PROJECT_FILES);
  //    mfolderContextMenu->addAction(cmd, Constants::G_FOLDER_FILES);
  ...
  cmd = Core::ActionManager::registerAction(d->m_addNewSubprojectAction,
                                            ProjectExplorer::Constants::ADDNEWSUBPROJECT,
                                            projecTreeContext);
  //    mprojectContextMenu->addAction(cmd, Constants::G_PROJECT_FILES);
  //    msubProjectContextMenu->addAction(cmd, Constants::G_PROJECT_FILES);
  ...
  cmd = Core::ActionManager::registerAction(d->m_removeFileAction,
                                            ProjectExplorer::Constants::REMOVEFILE,
                                            projecTreeContext);
  ...
  cmd = Core::ActionManager::registerAction(d->m_removeProjectAction,
                                            ProjectExplorer::Constants::REMOVEPROJECT,
                                            projecTreeContext);
  msubProjectContextMenu->addAction(cmd, Constants::G_PROJECT_FILES);

  // delete file action
  d->m_deleteFileAction = new QAction(tr("Delete File..."), this);
  cmd = Core::ActionManager::registerAction(d->m_deleteFileAction,
                                            ProjectExplorer::Constants::DELETEFILE,
                                            projecTreeContext);
  cmd->setDefaultKeySequence(QKeySequence::Delete);
  mfileContextMenu->addAction(cmd, Constants::G_FILE_OTHER);
  ...
  cmd = Core::ActionManager::registerAction(d->m_renameFileAction,
                                            ProjectExplorer::Constants::RENAMEFILE,
                                            projecTreeContext);
  mfileContextMenu->addAction(cmd, Constants::G_FILE_OTHER);
  ...
  d->m_setStartupProjectAction = new Utils::ParameterAction(tr("Set as Active Project"),
                                                            ...
                                                            Utils::ParameterAction::AlwaysEnabled, this);
  cmd = Core::ActionManager::registerAction(d->m_setStartupProjectAction,
                                            ProjectExplorer::Constants::SETSTARTUP,
                                            projecTreeContext);
  cmd->setAttribute(Core::Command::CA_UpdateText);
...
void ProjectExplorerPlugin::buildQueueFinished(bool success)
{
  ...
  if(success)
    this->d->m_buildSuccessFinished=true;
...
int ProjectExplorerPlugin::queue(QList<Project *> projects, QList<Core::Id> stepIds)
{
  ...
  this->d->m_buildSuccessFinished=false;
...
bool ProjectExplorerPlugin::coreAboutToClose()
{
  if (d->m_buildManager->isBuilding()) {
    ...
    box.setDefaultButton(cancelClose);
    box.setWindowTitle(tr("Close MBT Creator?"));
    box.setText(tr("A project is currently being built."));
    box.setInformativeText(tr("Do you want to cancel the build process and close MBT Creator anyway?"));
    box.exec();
    if (box.clickedButton() != closeAnyway)
      return false;
  }
                                                ...
                                                const bool forceSkipDeploy)
{
  ...
  if(queueCount==0)
  {
    this->executeRunConfiguration(rc, runMode);
    this->d->m_buildSuccessFinished=false;
    d->m_runMode = NoRunMode;
    d->m_delayedRunConfiguration = 0;
  }
  ...
  {
    if(this->d->m_buildSuccessFinished)//still successful finished
    {
      this->executeRunConfiguration(rc, runMode);
      this->d->m_buildSuccessFinished=false;
      d->m_runMode = NoRunMode;
...
void ProjectExplorerPlugin::updateContextMenuActions()
{
  ...
  d->m_renameFileAction->setEnabled(true);
  ...
  if (d->m_currentNode && d->m_currentNode->projectNode()) {
    ...
    if (qobject_cast<FolderNode*>(d->m_currentNode)) {
      ...
      d->m_removeFileAction->setVisible(enableRemove);
...
void ProjectExplorerPlugin::removeFile()
{
  ...
  QString fileEnd = filePath.split(QStringLiteral(".")).last();

  if(fileEnd == QStringLiteral("sut") ||
     fileEnd == QStringLiteral("testsystem") ||
     fileEnd == QStringLiteral("decl") ||
     fileEnd == QStringLiteral("irdldecl") ||
     fileEnd == QStringLiteral("opcuamap")
     ){
    QMessageBox::warning(Core::ICore::mainWindow(), tr("File is not removeable"),
                         tr("This file is requiered for the project. \nIt could not be removed."));
    return;
  }
